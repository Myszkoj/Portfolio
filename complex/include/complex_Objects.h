...
namespace complex
{
	...
	*/
	...
	public: // interface
		...
		virtual InstancePack*		get_object_as_instance_pack(const std::string&	OBJECT_NAME) = 0;
	...
	public: // functions
		...
		{
			return guess_index_from_byte(reinterpret_cast<const char*>(OBJECT_MEMBER));
		}
	};
...
namespace complex
{
	...
	private: // subtypes
		...
		using	MyParentT				= Parent<ChildT, BondT>;
		using	MyBaseT					= dpl::Association<ChildT, MyParentT, BOND_ID>;
		using	MyLinkT					= dpl::Association<MyParentT, ChildT, BOND_ID>;
	...
	private: // functions
		...
		inline ParentHandleT	get_parent_handle() const
		{
			...
			return PACK? ParentHandleT(parentType(), PACK->guess_object_name<MyParentT>(MyBaseT::other())) : ParentHandleT();
		}
		...
		void					set_parent(		const ParentHandleT&	PARENT_HANDLE)
		{
			...
			{
				if(MyParentT* parent = PARENT_HANDLE.acquire_parent(*pack))
				{
	...
	private: // subtypes
		...
		using	MyParentT		= Parent<ChildT, BondT>;
		using	MyLinkT			= dpl::Link<MyParentT, ChildT, BOND_ID>;
		using	MyChainT		= dpl::Chain<MyParentT, ChildT, BOND_ID>;
	...
	private: // functions
		...
		{
			parentType().throw_if_different_typeID<ParentT>();
			return static_cast<ParentT&>(*MyLinkT::get_chain());
		}
		...
		{
			parentType().throw_if_different_typeID<ParentT>();
			return static_cast<const ParentT&>(*MyLinkT::get_chain());
		}
		...
		{
			const ObjectPack* PACK = ObjectManager::ref().find_base_variant(parentType().ID);
			return PACK? ParentHandleT(parentType(), PACK->guess_object_name<MyParentT>(MyLinkT::get_chain())) : ParentHandleT();
		}
...
namespace complex
{
	...
	*/
	...
	{
	public: // subtypes
		using State		= InstancePack::AttachmentOperation;
	};
...
namespace complex
{
	...
	public: // functions
		...
		inline InstancePack*				find_instance_pack(	const InstancePack::Handle& HANDLE)
		{
		...
		inline InstancePack&				get_instance_pack(	const InstancePack::Handle& HANDLE)
		{
	...
	*/
	...
	public: // subtypes
		...
		using	ObjIndex				= uint32_t;
		using	NumObjects				= uint32_t;
		using	InvokeOBJ				= std::function<void(ObjT&)>;
		using	InvokeOBJs				= std::function<void(ObjT*, NumObjects)>;
		using	InvokeConstOBJ			= std::function<void(const ObjT&)>;
		using	InvokeConstOBJs			= std::function<void(const ObjT*, NumObjects)>;
		using	InvokeIndexedOBJ		= std::function<void(ObjT&, ObjIndex)>;
		using	InvokeIndexedConstOBJ	= std::function<void(const ObjT&, ObjIndex)>;
	...
	public: // iteration functions
		inline void					for_each(					const InvokeOBJ&				INVOKE)
		{
			std::for_each(m_objects.begin(), m_objects.end(), INVOKE);
		}

		inline void					for_each(					const InvokeOBJs&				INVOKE)
		{
			INVOKE(m_objects.data(), size());
		}

		inline void					for_each(					const InvokeConstOBJ&			INVOKE) const
		{
			std::for_each(m_objects.begin(), m_objects.end(), INVOKE);
		}

		inline void					for_each(					const InvokeConstOBJs&			INVOKE) const
		{
			INVOKE(m_objects.data(), size());
		}

		inline void					for_each(					const InvokeIndexedOBJ&			INVOKE)
		{
		...
		inline void					for_each(					const InvokeIndexedConstOBJ&	INVOKE)
		{
			const uint32_t SIZE = size();
			for(uint32_t index = 0; index < SIZE; ++index)
			{
	...
	private: // interface
		...
		virtual InstancePack*		get_object_as_instance_pack(const std::string&				OBJECT_NAME) final override
		{
			if constexpr (std::is_base_of_v<InstancePack, ObjT>) return find(OBJECT_NAME);
			else return nullptr;
		}
...
{
	template<>
	struct	Traits<test123::MyObject>
	{
		using ParentTypes		= dpl::TypeList<>;
		using ChildTypes		= dpl::TypeList<>;
		using ComponentTypes	= dpl::TypeList<test123::ComponentA, test123::ComponentB>;
	};
}

namespace test123
{
	class MyObject : public complex::Object<MyObject>
	{
