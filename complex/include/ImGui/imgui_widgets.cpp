...
static uint32_t inputScalarBufferSize = 64;
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
	...
	char buf[64];
	DataTypeFormatString(buf, inputScalarBufferSize, data_type, p_data, format); // IM_ARRAYSIZE(buf)
	...
	if (p_step != NULL)
	{
		...
		if (InputText("", buf, inputScalarBufferSize, flags)) // PushId(label) + "" gives us the expected ID from outside point of view
	...
	{
		if (InputText(label, buf, inputScalarBufferSize, flags))
			value_changed = DataTypeApplyOpFromText(buf, g.InputTextState.InitialTextA.Data, data_type, p_data, format);
	}
	if (value_changed)
...
void ImGui::SetInputScalarBuffer(const unsigned int SIZE)
{
	inputScalarBufferSize = (SIZE < 64) ? SIZE : 64;
}
