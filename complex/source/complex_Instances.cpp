...
namespace complex
{
	void	InstancePack::AttachmentOperation::attach(	InstancePack& pack)
	{
	...
	void	InstancePack::AttachmentOperation::detach(	InstancePack& pack)
	{
		auto& group = InstanceManager::ref().get_group(groupName);
		state->reset_out();
...
namespace complex
{
	CLASS_CTOR	InstancePack::AttachCommand::AttachCommand(	const InstanceGroup&	GROUP,
															const InstancePack&		PACK)
		: AttachCommand(GROUP.get_label(), PACK.query_handle())
	{

	}

	bool		InstancePack::AttachCommand::valid() const
	{
	...
	void		InstancePack::AttachCommand::execute()
	{
		AttachmentOperation::attach(ObjectManager::ref().get_instance_pack(packHandle));
	}

	void		InstancePack::AttachCommand::unexecute()
	{
...
namespace complex
{
	CLASS_CTOR	InstancePack::DetachCommand::DetachCommand(	const InstancePack&		PACK)
		: AttachmentOperation(PACK.is_linked()? PACK.get_chain()->get_label() : "")
	{
		packHandle	= PACK.query_handle();
	}

	bool		InstancePack::DetachCommand::valid() const
	{
	...
	void		InstancePack::DetachCommand::execute()
	{
		AttachmentOperation::detach(ObjectManager::ref().get_instance_pack(packHandle));
	}

	void		InstancePack::DetachCommand::unexecute()
	{
		AttachmentOperation::attach(ObjectManager::ref().get_instance_pack(packHandle));
	}
}
...
namespace complex
{
	...
	void			InstanceGroup::attach_instance_pack(	InstancePack&				pack,
	...
	void			InstanceGroup::detach_instance_pack(	InstancePack&				pack,
	...
	{
		Chain::for_each([&](InstancePack& pack)
		{
	...
	void			InstanceGroup::import_all_from(			std::istream&				binaryState)
	{
		...
		InstancePack::Handle	packHandle;

		for(uint32_t index = 0; index < NUM_PACKS; ++index)
		{
			packHandle.import_from(binaryState);
			InstancePack* pack = ObjectManager::ref().find_instance_pack(packHandle);
	...
	void			InstanceGroup::export_all_to(			std::ostream&				binaryState)
	{
		...
		Chain::for_each([&](const InstancePack& PACK)
		{
