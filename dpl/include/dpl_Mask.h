...
namespace dpl
{
	...
	inline uint32_t		propagateBits(		uint32_t mask) 
	{
		mask |= mask >> 1;
		mask |= mask >> 2;
		mask |= mask >> 4;
	...
	inline uint8_t		highestSetBitIndex8(uint32_t mask)
	{
		return DE_BRUIJN_BIT_TABLE[(propagateBits(mask) * 0x07C4ACDDu) >> 27];
	}


	template<typename UIntT, typename EnumT = UIntT> requires std::is_integral_v<UIntT> && std::is_unsigned_v<UIntT>
	class Mask
	{
	public: // constants
	...
	public: // lifecycle
		...
		CLASS_CTOR		Mask(			const IndexRange<UIntT>			SET_BIT_RANGE)
			: m_bits(0)
		{
			set_range(SET_BIT_RANGE, true);
		}
	...
	public: // functions
		...
		*/
		...
		inline void		set_range(		const IndexRange<UIntT>			SET_BIT_RANGE,
										const bool						bVALUE)
		{
			if(SET_BIT_RANGE.begin() < MAX_NUM_BITS)
			{
				const UIntT END = (SET_BIT_RANGE.end() <= MAX_NUM_BITS)? SET_BIT_RANGE.end() : MAX_NUM_BITS;
				for(UIntT index = SET_BIT_RANGE.begin(); index < END; ++index)
				{
					set_at(static_cast<EnumT>(index), bVALUE);
				}
			}
		}
