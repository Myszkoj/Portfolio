	...
	*/
	...
	public: // functions
		...
		*/
		...
		inline void				for_each_entry(		std::function<void(EntryT&)>		function)
		{
			for(auto& key : *entries)
			{		
				function(static_cast<EntryT&>(*key.target()));
			}
		}

		inline void				for_each_entry(		std::function<void(const EntryT&)>	function) const
		{
			for(const auto& KEY : entries())
			{		
				function(static_cast<const EntryT&>(*KEY.target()));
			}
		}
